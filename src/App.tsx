...
type Tok = { type: 'num'; v: bigint } | { type: 'op'; v: string } | { type: 'lpar' } | { type: 'rpar' };
...
function tokenize(expr: string): Tok[] {
	...
	while (i < s.length) {
		...
		if (ch === '0' && (s[i + 1]?.toLowerCase() === 'x' || s[i + 1]?.toLowerCase() === 'b' || s[i + 1]?.toLowerCase() === 'o')) {
			...
			toks.push({ type: 'num', v: BigInt(lit) });
			i = j; continue;
		}
		// bare hex/dec with optional h/b suffix
		if (/[0-9a-f]/i.test(ch)) {
			let j = i;
			while (j < s.length && /[0-9a-f]/i.test(s[j])) j++;
			if (s[j]?.toLowerCase() === 'h') {
				toks.push({ type: 'num', v: BigInt('0x' + s.slice(i, j)) });
				i = j + 1; continue;
			}
			if (s[j]?.toLowerCase() === 'b' && /^[01]+$/i.test(s.slice(i, j))) {
				toks.push({ type: 'num', v: BigInt('0b' + s.slice(i, j)) });
				i = j + 1; continue;
			}
			// decimal by default
			toks.push({ type: 'num', v: BigInt(s.slice(i, j)) });
			i = j; continue;
		}
...
function evalRPN(rpn: Tok[], widthMask?: bigint): bigint {
	const st: bigint[] = [];
	const mask = widthMask ?? undefined;
	const applyMask = (v: bigint) => (mask !== undefined ? (v & mask) : v);
	for (const t of rpn) {
		if (t.type === 'num') st.push(applyMask(t.v));
		else if (t.type === 'op') {
			const op = t.v;
			if (OPS[op].arity === 1) {
				const a = st.pop(); if (a === undefined) throw new Error('Stack underflow');
				let r: bigint;
				if (op === '~') r = ~a;
				else if (op === 'u-') r = -a;
				else throw new Error('Unknown unary op');
				st.push(applyMask(r));
			} else {
				const b = st.pop(); const a = st.pop(); if (a === undefined || b === undefined) throw new Error('Stack underflow');
				let r: bigint;
				switch (op) {
					case '+': r = a + b; break;
					case '-': r = a - b; break;
					case '*': r = a * b; break;
					case '/': if (b === 0n) throw new Error('Division by zero'); r = a / b; break;
...
function evalExpression(expr: string, width: number, maskExprToWidth: boolean) {
	try {
		...
		const val = evalRPN(rpn, m as any);
...
export default function RegisterWorkbench() {
	...
	const [maskExprToWidth, setMaskExprToWidth] = useState<boolean>(true);
		...
		<div className="min-h-screen w-full bg-neutral-50 text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100 p-6">
			<div className="mx-auto max-w-6xl grid md:grid-cols-5 gap-6">
				<header className="md:col-span-5">
					...
					<p className="text-sm opacity-80">Convert hex/decimal/binary, compute expressions with memory, toggle bits, view endianness, work with ASCII, and generate C-style masks.</p>
				</header>
				...
				<section className="md:col-span-3 space-y-3">
					...
					<div className="rounded-2xl p-4 bg-white dark:bg-neutral-900 shadow">
						<label className="text-sm font-medium">Value (hex/dec/bin accepted)</label>
				...
				{/* Bits + Labels + Endianness + Codegen + Tests + Help */}
